...
def quiz_kapitel_index(mainwindow):
    ...
    for i, item in enumerate(chapter_list):
        var = tk.BooleanVar()
        nm = "cb" + str(i)
        checkbox = tk.Checkbutton(second_frame, text=item, variable=var, name=nm) #"Checkbutton {}".format(i)
...
#
    ...
    close_button = tk.Button(second_frame, text="Speichern - Schließen", command=save_checkboxes_state)
    close_button.pack(side="bottom", padx=15, pady=15, anchor="center")
